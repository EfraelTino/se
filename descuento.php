<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Carrois+Gothic+SC&family=Raleway:ital,wght@0,100;0,200;0,300;0,400;0,500;0,600;0,700;0,800;0,900;1,100;1,300;1,400;1,500;1,600;1,700;1,800;1,900&display=swap" rel="stylesheet">
<style>
    .font-dm-sans {
        font-family: 'Raleway', sans-serif;
    }
</style>


<div class="descuentoContainer">
    <div class="descuento container">
        <div class="dentro font-dm-sans ">
            <h2 class="aprovecha">Aprovecha
                <br /> 50% descuento
            </h2>
            <style>
                .compra:hover{
                    text-decoration: none;
                }
            </style>
            <a href="productos.php" class="button-descuento no-underline compra"> Compra Ahora</a>
        </div>
    </div>
</div>